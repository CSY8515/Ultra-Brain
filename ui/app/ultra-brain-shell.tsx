      ...
      {panel === "studio" && <>
        ...
        <aside className={`settings-drawer is-open ${studioTab === "custom" ? "is-canvas" : ""}`} aria-label="UI Studio">
          ...
          {studioTab === "themes" && <div className="studio-pane">
            ...
            <fieldset><legend>테마 브라우저</legend><div className="theme-grid">{themeNames.map((theme) => { const item = themeRegistry[theme]; const world = profile.worldEngine && activePreference.theme === theme ? profile.worldEngine : resolveThemeProfile({ ...activePreference, theme }).worldEngine; return <button key={theme} type="button" className={activePreference.theme === theme ? "is-selected" : ""} onClick={() => selectTheme(theme)}><span className={`theme-preview ${theme}`} style={{ backgroundImage: `${world.background}, url(/ultra-brain-world.png)`, filter: `${item.worldFilter} contrast(${profile.adjustments.contrast}) saturate(${profile.adjustments.saturation})` }} /><strong>{item.label}</strong><small>{world.label} · {world.motion === "still" ? "정적" : "움직임"}</small></button>; })}</div></fieldset>
