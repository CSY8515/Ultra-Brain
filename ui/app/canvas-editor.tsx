...
const COMPONENTS: { id: BuilderTool; label: string }[] = [
  { id: "frame", label: "Frame" }, { id: "container", label: "Container" }, { id: "text", label: "글자" }, { id: "button", label: "버튼" },
...
export function CanvasEditor({ baseTheme, onUseTheme, onToast }: CanvasEditorProps) {
  ...
  const [openToolboxes, setOpenToolboxes] = useState<Record<ToolboxKey, boolean>>({ beginner: true, intermediate: false, advanced: false, expert: false, touch: false, mouse: true });
  ...
  function toggleToolbox(key: ToolboxKey) { setOpenToolboxes((current) => ({ ...current, [key]: !current[key] })); }
  function addBuilderNode(kind: BuilderTool, point: Point = { x: 120 + (nodesRef.current.length % 3) * 280, y: 100 + (nodesRef.current.length % 4) * 120 }) {
    ...
    const labels: Record<string, string> = { frame: "Frame", container: "Container", text: textValue || "글자", button: "버튼", card: "카드", panel: "패널", widget: "위젯", navigation: "탐색 버튼", icon: "아이콘", shape: "도형" };
  ...
  return <div className={`canvas-editor preview-${previewMode} builder-${builderMode}`} style={shellStyle} onClick={() => contextMenu && setContextMenu(null)}>
    <div className="canvas-editor-header"><div><small>UI BUILDER · {draftSavedAt ? `저장 ${new Date(draftSavedAt).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}` : "초안 없음"}</small><h3>UI Builder</h3><p>화면을 만들고 구성 요소와 에셋을 재사용하세요. 저장 전 미리 확인하고 언제든 이전 리비전으로 되돌릴 수 있습니다.</p></div><div className="canvas-header-actions"><div className="canvas-mode-row"><button type="button" className={builderMode === "builder" ? "is-selected" : ""} onClick={() => setBuilderMode("builder")}>UI 만들기</button><button type="button" className={builderMode === "drawing" ? "is-selected" : ""} onClick={() => setBuilderMode("drawing")}>그리기 보조</button>{(["desktop", "tablet", "mobile", "full"] as const).map((mode) => <button key={mode} type="button" className={previewMode === mode ? "is-selected" : ""} onClick={() => setPreviewMode(mode)}>{mode === "desktop" ? "데스크톱" : mode === "tablet" ? "태블릿" : mode === "mobile" ? "모바일" : "전체 화면"}</button>)}</div><div className="canvas-value-row"><button type="button" onClick={undo} disabled={!history.length}>되돌리기</button><button type="button" onClick={redo} disabled={!redoStack.length}>다시 실행</button><button type="button" onClick={saveDraft}>초안 저장</button><button type="button" className="canvas-primary-action" onClick={() => saveRevision("applied")}>적용 저장</button></div></div></div>
    <div className="builder-workspace">
      <aside className="builder-tool-panel">
        {section("beginner", "초보자용 UI 제작", "처음 시작하는 화면", <><button type="button" onClick={newScreen}>새 화면 만들기</button><label className="builder-file-action"><input type="file" multiple accept="image/*" onChange={(event) => importAsset(event, "asset")} /><span>그림 가져오기</span></label><label className="builder-file-action"><input type="file" accept="image/*" onChange={(event) => importAsset(event, "background")} /><span>배경 바꾸기</span></label><label className="builder-file-action"><input type="file" accept="image/*" onChange={(event) => importAsset(event, "icon")} /><span>아이콘 가져오기</span></label><label className="builder-file-action"><input type="file" accept="image/*" onChange={(event) => importAsset(event, "photo")} /><span>사진 가져오기</span></label><label className="builder-file-action"><input type="file" accept="image/*" onChange={(event) => importAsset(event, "ai")} /><span>AI 이미지 가져오기</span></label><button type="button" onClick={() => addBuilderNode("button")}>버튼 추가</button><button type="button" onClick={() => addBuilderNode("text")}>글자 추가</button><button type="button" onClick={() => setInteractionMode("select")}>위치 바꾸기</button><button type="button" onClick={() => selectedNode && updateNode(selectedNode.id, { width: (selectedNode.width || 240) + 20, height: (selectedNode.height || 66) + 10 })}>크기 바꾸기</button><button type="button" onClick={deleteSelected} disabled={!selectedIds.length}>삭제</button><button type="button" onClick={undo} disabled={!history.length}>되돌리기</button><button type="button" onClick={saveDraft}>저장</button><button type="button" onClick={() => saveRevision("applied")}>적용</button></>)}
        ...
        {section("advanced", "고급자용 UI 제작", "정교한 구조", <><div className="toolbox-chip-row">{COMPONENTS.map((item) => <button key={item.id} type="button" className={builderTool === item.id ? "is-selected" : ""} onClick={() => { setBuilderMode("builder"); setBuilderTool(item.id); }}>{item.label}</button>)}</div><button type="button" onClick={() => setBuilderTool("frame")}>Frame 추가</button><button type="button" onClick={() => setBuilderTool("container")}>Container 추가</button><button type="button" onClick={() => onToast("Responsive 미리보기는 상단 장치 버튼으로 확인합니다")}>Responsive 확인</button></>)}
        {section("expert", "전문가용 UI 제작", "구조와 규칙", <><div className="expert-readout"><span>Component Tree</span><b>{nodes.length}개</b><span>Registry</span><b>local</b><span>Design Token</span><b>{color}</b><span>State</span><b>{componentState}</b><span>Version</span><b>v0.97</b></div><button type="button" onClick={() => onToast("현재 UI 패키지를 검사했습니다")}>호환성 검사</button><button type="button" onClick={() => saveRevision("manual")}>리비전 만들기</button></>)}
