  ...
  return <div className={`canvas-editor preview-${previewMode} builder-${builderMode} ${previewing ? "is-previewing" : ""}`} style={shellStyle} onClick={() => contextMenu && setContextMenu(null)}>
    ...
    <div className="builder-workspace">
      <aside className="builder-tool-panel">
        <section className="studio-quickstart"><div className="canvas-section-heading"><strong>처음이라면 여기부터</strong><span>{nodes.length ? "편집 중" : "1분 시작"}</span></div><p>아래 순서대로 누르면 첫 화면이 만들어집니다.</p><div className="quickstart-steps"><button type="button" onClick={newScreen}><b>1</b><span>새 화면 만들기<small>빈 화면으로 시작</small></span></button><label className="quickstart-step"><input type="file" multiple accept="image/*" onChange={(event) => importAsset(event, "asset")} /><b>2</b><span>그림 가져오기<small>사진·배경 추가</small></span></label><button type="button" onClick={() => addBuilderNode("button")}><b>3</b><span>버튼 넣기<small>화면에 요소 추가</small></span></button><button type="button" onClick={() => saveRevision("applied")}><b>4</b><span>저장하고 적용<small>완성한 화면 사용</small></span></button></div><small className="quickstart-tip">도구는 다시 누르면 선택이 해제됩니다.</small></section>
        {section("beginner", "기본 도구", "처음이면 여기서 시작", <><button type="button" onClick={newScreen}>새 화면 만들기</button><label className="builder-file-action"><input type="file" multiple accept="image/*" onChange={(event) => importAsset(event, "asset")} /><span>그림 가져오기</span></label><button type="button" onClick={() => addBuilderNode("button")}>버튼 추가</button><button type="button" onClick={() => addBuilderNode("text")}>글자 추가</button><button type="button" onClick={() => setInteractionMode("select")}>위치 바꾸기</button><button type="button" onClick={() => selectedNode && updateNode(selectedNode.id, { width: (selectedNode.width || 240) + 20, height: (selectedNode.height || 66) + 10 })}>크기 바꾸기</button><button type="button" onClick={deleteSelected} disabled={!selectedIds.length}>삭제</button><button type="button" onClick={undo} disabled={!history.length}>되돌리기</button><button type="button" onClick={saveDraft}>저장</button><button type="button" onClick={() => saveRevision("applied")}>적용</button></>)}
        {section("intermediate", "배치와 효과", "정렬·잠금·움직임", <><button type="button" onClick={() => alignSelected("x")} disabled={selectedIds.length < 2}>자동 정렬</button><button type="button" onClick={groupSelected} disabled={selectedIds.length < 2}>그룹 만들기</button><button type="button" onClick={distributeSelected} disabled={selectedIds.length < 3}>간격 맞추기</button><button type="button" onClick={() => selectedNode && updateNode(selectedNode.id, { locked: !selectedNode.locked })} disabled={!selectedNode}>{selectedNode?.locked ? "잠금 해제" : "잠금"}</button><button type="button" onClick={() => setComponentState("Hover")}>마우스 효과</button><button type="button" onClick={() => setComponentState("Active")}>눌렀을 때 효과</button><button type="button" onClick={() => onToast("화면 전환 설정을 준비했습니다")}>화면 전환</button></>)}
        {section("advanced", "구조 추가", "틀과 부품", <><div className="toolbox-chip-row">{COMPONENTS.map((item) => <button key={item.id} type="button" className={builderTool === item.id ? "is-selected" : ""} onClick={() => { setBuilderMode("builder"); setBuilderTool(item.id); }}>{item.label}</button>)}</div><button type="button" onClick={() => setBuilderTool("frame")}>틀 추가</button><button type="button" onClick={() => setBuilderTool("container")}>묶음 추가</button><button type="button" onClick={() => onToast("화면 크기를 바꿔도 배치를 확인할 수 있습니다")}>반응형 확인</button></>)}
        {section("expert", "세부 정보", "필요할 때만 열기", <><div className="expert-readout"><span>요소 트리</span><b>{nodes.length}개</b><span>보관함</span><b>이 기기</b><span>디자인 값</span><b>{color}</b><span>상태</span><b>{COMPONENT_STATE_LABELS[componentState] || componentState}</b><span>버전</span><b>v0.982</b></div><button type="button" onClick={() => onToast("현재 UI를 확인했습니다")}>호환성 검사</button><button type="button" onClick={() => saveRevision("manual")}>변경 기록 만들기</button></>)}
        {section("touch", "그리기 도구", "펜과 그림", <><div className="toolbox-chip-row">{DRAWING_TOOLS.slice(0, 6).map((item) => <button key={item.id} type="button" className={tool === item.id ? "is-selected" : ""} onClick={() => { setBuilderMode("drawing"); setTool(item.id); }}>{item.label}</button>)}</div><label>붓 크기 <input type="range" min="1" max="80" value={size} onChange={(event) => setSize(Number(event.target.value))} /></label><label>투명도 <input type="range" min="0.1" max="1" step="0.05" value={opacity} onChange={(event) => setOpacity(Number(event.target.value))} /></label><label>안정화 <input type="range" min="0" max="0.8" step="0.05" value={stabilizer} onChange={(event) => setStabilizer(Number(event.target.value))} /></label><button type="button" className={pressureEnabled ? "is-selected" : ""} onClick={() => setPressureEnabled((current) => !current)}>압력 반영 {pressureEnabled ? "켜짐" : "꺼짐"}</button><button type="button" onClick={() => setCanvasRotation((current) => current - 15)}>캔버스 왼쪽 회전</button><button type="button" onClick={() => setCanvasRotation((current) => current + 15)}>캔버스 오른쪽 회전</button></>)}
        {section("mouse", "마우스 도구", "정밀하게 옮기기", <><button type="button" className={interactionMode === "select" ? "is-selected" : ""} onClick={() => setInteractionMode("select")}>정밀 이동</button><button type="button" className={snap ? "is-selected" : ""} onClick={() => setSnap((current) => !current)}>스냅</button><button type="button" className={smartGuide ? "is-selected" : ""} onClick={() => setSmartGuide((current) => !current)}>스마트 가이드</button><button type="button" onClick={() => setSelectedIds(nodesRef.current.map((node) => node.id))}>여러 개 선택</button><button type="button" onClick={() => alignSelected("x")} disabled={selectedIds.length < 2}>맞춤 정렬</button><button type="button" onClick={duplicateSelected} disabled={!selectedIds.length}>복제</button><button type="button" className={interactionMode === "pan" ? "is-selected" : ""} onClick={() => setInteractionMode("pan")}>캔버스 이동</button><button type="button" onClick={() => setCanvasOffset({ x: 0, y: 0 })}>화면 위치 초기화</button></>)}
